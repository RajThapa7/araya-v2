...
const Page = ({ params }: { params: { slug: string } }) => {
  ...
  const { data, isLoading } = useFetchProductUnderCategories(slug);

  const currentCategory = data?.data[0]?.category;
    ...
    <div className="flex-flex-col">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mt-4">
        <div className="flex flex-col gap-2">
          <p className="text-body font-semibold">Category Name</p>
          <p>{currentCategory?.categoryName}</p>
        </div>
        <div className="flex flex-col gap-2">
          <p className="text-body font-semibold">Category Slug</p>
          <p>{currentCategory?.slug}</p>
        </div>
        <div className="flex flex-col gap-2">
          <p className="text-body font-semibold">Created at</p>
          <p>{new Date(currentCategory?.createdAt || "").toString()}</p>
        </div>
        <div className="flex flex-col gap-2">
          <p className="text-body font-semibold">Updated at</p>
          <p>{new Date(currentCategory?.updatedAt || "").toString()}</p>
        </div>
      </div>
      {/* details */}
